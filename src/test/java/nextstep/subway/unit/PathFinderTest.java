...
class PathFinderTest {
    ...
    void setUp() {
        ...
        Section section5 = Section.createSection(다른노선, 신논현역, 판교역, 30); // 환승 구간

        List<Section> allSections = Arrays.asList(section1, section2, section3, section4, section5);

        // PathFinder 생성
        pathFinder = PathFinder.createPath(graph, allSections, allStations);
    }

    @Test
    @DisplayName("경로 그래프 setting 테스트")
    void createPathWithMultipleLinesTest() {
        // 검증
        ...
        assertThat(graph.containsEdge(2L, 4L)).isTrue(); // 환승 구간
    }

    @Test
    @DisplayName("최소 길이 경로 찾기 테스트")
    ...
    @Test
    @DisplayName("연결되지 않은 경로 테스트")
    void getShortestPathNoRouteTest() {
        // 연결되지 않은 역 추가
        Station 고립역 = new Station("고립역");
        ReflectionTestUtils.setField(고립역, "id", 6L);
        graph.addVertex(6L);

        PathResult shortestPath = pathFinder.getShortestPath(1L, 6L);

        assertThat(shortestPath.getPathStationIds()).isEmpty();
    }
}
